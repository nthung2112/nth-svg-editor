 ...
 * @param callback callback after setup
 */
export function setupModalHandler(modalSelector: string, content: string, callback?: () => void) {
  ...
  if (content) {
    ...
    if (modalBody) {
      ...
      (modalElement.querySelector('[data-control-btn="dialog"]') as HTMLElement)!.style.display =
        "none";
    }
  }
...
async function copyToClipboard(btn: HTMLElement): Promise<void> {
  try {
    const svgEleList = btn.querySelectorAll("svg");
    const text: string = btn.getAttribute("data-clipboard-code") || "";
    const safeValue = decodeURIComponent(text);
    const originalCode = decompress(decrypt(safeValue));
    if (navigator.clipboard?.writeText) {
      await navigator.clipboard.writeText(originalCode);
      const copyIcon = svgEleList[0];
      const checkIcon = svgEleList[1];
 ...
 * @param callback callback after replace
 */
async function replaceImageWithSvg(
  diagram: HTMLElement,
  svgTemp: any,
  callback: () => void
): Promise<void> {
...
async function svgToRough(
  ...
  const initIcon = btnSelectors[0];
  const loadingIcon = btnSelectors[1];
  const checkIcon = btnSelectors[2];

  btnSelectors.forEach((btnSelector, index) => {
    btnSelector.classList.toggle("fg-none", index !== 1);
  });
...
/**
 * @param selector
 * [data-control-panel-container-modal] | [data-control-panel-container]
 */
const markdownItDiagramDom: (selector?: ContainterSelector) => void = (
  selector = "[data-control-panel-container]"
) => {
  ...
  // setup modal handler
  if (!document.querySelector(`[${SelectorEnum.MODAL}]`)) diagramModal = new DiagarmModal();
  containers.forEach((container: Element) => {
    ...
    const handleButtonClick = (event: Event): void => {
      const button: HTMLElement | null = event.target
        ? (event.target as HTMLElement).closest("[data-control-btn]")
        : null;
      ...
      const actionMap: ActionMap = {
        ...
        dialog: () => {
          setupModalHandler(
            `[${SelectorEnum.MODAL}]`,
            `<div data-control-panel-container-modal>${container.innerHTML}</div>`,
            () => {
              diagramModal?._showModal();
              markdownItDiagramDom("[data-control-panel-container-modal]");
            }
          );
        },
